...
from videollama3 import model_init, model_init_crossview, mm_infer, disable_torch_init
...
def model_generation(tokenizer, model, image_processor, image_list, video_list, prompt, modality='image'):
    prompt = prompt.replace('<image0>\n', '') # viscop processor doesnt require the token
    ...
    if modality == 'image':
        ...
        timestamps = [float(i+1) for i in range(num_frames)]
    ...
    # will process frames or images depending on the modality, but both called images
    inputs = image_processor(
        images=[images],
        text=conversation,
        merge_size=2 if modality == "video" else 1,
        return_tensors="pt",
    )
...
def safe_write_buffer(path: str, buffer: dict):
    """
    Safely writes a buffer of data to a JSON file. Used during training to keep all processes up to date with latest data

    Args:
        path (str): Path to the JSON file
        buffer (dict): Buffer containing the new data to write
    """
...
def safe_write_json(path: str, data: dict):
    """
    Safely writes a JSON file. Used before/after training to write the final json
...
def eval_episode(args, gen_action_fn, parse_action_fn, global_rank=0, world_size=1, skip_error_tasks=False):
    """
    ...
    # Load VideoLlama model
    disable_torch_init()
    config = json.load(open(os.path.join(args.model_path, 'config.json')))
    if 'num_latent_queries' in config:
        _model_init = model_init_crossview
    else:
        _model_init = model_init
    
    print(f'Using model: {_model_init}')

    model, image_processor = _model_init(
    ...
    partition_list = [args.partition] if args.partition else ALL_PARTITIONS # ['placement_generalization', 'combinatorial_generalization', 'novel_object_generalization', 'novel_task_generalization']
    partition_list = partition_list[:3] # filter out L4, llara doesnt use it in its evaluations
    episodes = []
    eval_tasks = set()

    if skip_error_tasks:
        skip_tasks = ['rearrange', 'rearrange_then_restore', 'follow_order', 'twist', 'manipulate_old_neighbor', 'same_shape', 'scene_understanding']
        
    for partition in partition_list:
        for task in PARTITION_TO_SPECS["test"][partition].keys():
            if skip_error_tasks and task in skip_tasks:
                print(f"Skipping task {task} in partition {partition}.")
                continue

            eval_tasks.add(task)

            for seed in seed_range:
                episodes.append((partition, task, seed))

    print(f"Tasks to be evaluated: {eval_tasks}")

    per_device = len(episodes) // world_size
    all_episodes_count = len(episodes)

    if global_rank == world_size - 1:
        episodes = episodes[global_rank * per_device:]
    else:
        episodes = episodes[global_rank * per_device:(global_rank + 1) * per_device]
    print(f"(process {global_rank}) I will perform {len(episodes)}/{all_episodes_count} episodes.")
    ...
    iterator = tqdm(enumerate(episodes), leave=False, total=len(episodes))
    ...
    for episode_idx, episode in iterator:
        partition, task, seed = episode
        tid = f'{partition}/{task}/{seed}'
        if tid in result:
            print(f'Already evaluated {tid}, skipping')
            continue
        ...
        for step in range(args.max_length):
            ...
            while not action_queue and retry < 1:
                parsed_action, prompt_txt, answer_txt = gen_action_fn(
                    ...
                    obs, detector, args.prompt_id, image_cache, args.modality
                )

                # if you want to draw the rollouts with the predicted action
                # try:
                #     image = obs['rgb']['front'].copy()
                #     image_4_cv2 = np.moveaxis(image, 0, -1)
                #     image_4_cv2 = cv2.cvtColor(image_4_cv2, cv2.COLOR_RGB2BGR)

                #     coord_1 = tuple(map(float, re.findall(r"[-+]?\d*\.\d+|\d+", parsed_action[0][0])))
                #     coord_2 = tuple(map(float, re.findall(r"[-+]?\d*\.\d+|\d+", parsed_action[0][1])))

                #     coord_1 *= np.array([256, 128])
                #     coord_2 *= np.array([256, 128])

                #     cv2.circle(image_4_cv2, (int(coord_1[0]), int(coord_1[1])), 5, (0, 255, 0), -1)
                #     cv2.circle(image_4_cv2, (int(coord_2[0]), int(coord_2[1])), 5, (0, 0, 255), -1)

                #     cv2.imwrite(f"./rollouts/{tid.replace('/', '_')}_step{step}.jpg", image_4_cv2)
                #     print('wrote rollout')
                # except:
                #     pass
                ...
                image_cache.append(obs['rgb']['front'].copy())
                retry += 1
        ...
        buffer[tid] = result[tid]

        if len(buffer) >= write_every or episode_idx == len(episodes) - 1:
            result = safe_write_buffer(filename, buffer)
            buffer = {}

        total_success += info['success']
        iterator.set_description(f"(P{global_rank}-E{episode_idx+1}/{len(episodes)}) | {total_success}/{episode_idx+1} ({total_success/(episode_idx+1) * 100:.1f}%) | {step+1} steps")

    # Cleanup and final write
    del model
    torch.cuda.empty_cache()
    assert len(buffer) == 0, "Buffer JSON should be empty at the end of evaluation!"