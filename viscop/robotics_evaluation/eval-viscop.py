...
def query_bc(tokenizer, model, image_processor, prompt, prompt_mode, prompt_assets, action_hist, obs, detector, prompt_id, prev_visual_observations=[], modality='image'):
    """
    Generate an action plan based on the prompt and current observation.
    ...
    # Create list of frames for video. prev observations + current observation
    video_list = prev_visual_observations + [obs['rgb']['front'].copy()]

    # Generate model output
    answer, _ = model_generation(tokenizer, model, image_processor, image_list, video_list, prepared_prompt, modality=modality)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-path', type=str, required=True,
                        help='Path to the VisCoP checkpoint.')
    ...
    parser.add_argument('--skip-error-tasks', action='store_true')
    ...
    parser.add_argument('--modality', type=str, default='image',
                        help='Modality to pass to the model. Options: "image", "video".')

    # Parse arguments and run evaluatio
    ...
    print(global_rank, local_rank, world_size)

    eval_episode(args, query_bc, parse_bc, global_rank, world_size, args.skip_error_tasks)