...
def safe_save_model_for_hf_trainer(trainer: Trainer,
    ...
    if getattr(trainer.args, "is_alignment", False):
        # Only save Adapter
        keys_to_match = ['mm_projector', 'visual_probes', 'interaction_modules']
...
class ViSCoP_Trainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.vision_encoder_lr is not None:
                ...
                # visual_probe_parameters = [name for name, _ in optimized_parameters if "visual_probes" in name]
                # decay_visual_probe_parameters = [name for name in visual_probe_parameters if name in decay_parameters]
                # nodecay_visual_probe_parameters = [name for name in visual_probe_parameters if name not in decay_parameters]
                # optimizer_grouped_parameters.extend([
                #     {
                #         "params": [p for n, p in optimized_parameters if n in decay_visual_probe_parameters],
                #         "weight_decay": self.args.weight_decay,
                #         "lr": 2e-6,
                #     },
                #     {
                #         "params": [p for n, p in optimized_parameters if n in nodecay_visual_probe_parameters],
                #         "weight_decay": 0.0,
                #         "lr": 2e-6,
                #     }
                # ])
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'is_alignment', False):
            ...
            # Only save Adapter
            keys_to_match = ['mm_projector', 'vision_resampler', 'visual_probes', 'interaction_modules']
            ...
            if self.args.lora_enable:
                ...
                super(ViSCoP_Trainer, self)._save_checkpoint(model, trial, metrics)
            else:
                super(ViSCoP_Trainer, self)._save_checkpoint(model, trial, metrics)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'is_alignment', False):
            ...
            super(ViSCoP_Trainer, self)._save(output_dir, state_dict)
