# Adpoted from https://github.com/DAMO-NLP-SG/VideoLLaMA3. Below is the original copyright:
...
from viscop.constants import (IGNORE_INDEX,
    NUM_FRAMES, DEFAULT_IMAGE_TOKEN, STREAM_MAX_FRAMES,
    STREAM_START_TOKEN, STREAM_END_TOKEN, DEFAULT_PROBE_TOKEN)
from viscop.mm_utils import (load_images, load_video)
from viscop.model import *
from viscop.viscop_trainer import (
    ViSCoP_Trainer, find_all_linear_names, get_peft_state_maybe_zero_3,
    get_peft_state_non_lora_maybe_zero_3, safe_save_model_for_hf_trainer)
from viscop.model.processor import ViSCoP_Processor
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="viscop_qwen2", metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
    model_path: Optional[str] = field(default="lmsys/vicuna-7b-v1.5")
    version: Optional[str] = field(default="v1", metadata={"help": "Version of the conversation template."})
    freeze_backbone: bool = field(default=False, metadata={"help": "Whether to freeze the LLM backbone."})
    # If wanting to continue training on trained LoRA model
    pretrained_lora_model: Optional[str] = field(default=None)
    # Connector Arguments
    ...
    include_visual_tokens: Optional[bool] = field(default=False, metadata={"help": "Whether to include the visual features as input to the LLM."})
    include_visual_probes: Optional[bool] = field(default=False, metadata={"help": "Whether to include the visual probes from ViSCoP as input to the LLM."})
    num_visual_probes: Optional[int] = field(default=64, metadata={"help": "Number of visual probes in ViSCoP."})
    interaction_module_layers: Optional[str] = field(default=None, metadata={"help": "Controls the locations of interaction modules in ViSCoP. Input should be comma separated list of integers (zero-indexed) (valid layers with SigLIP 0-26). Passing the string \"None\" will place interaction modules after every layer."})
    interaction_module: Optional[str] = field(default=None, metadata={"help": "Type of attention/interaction_module to use"})
    warm_init_interaction_module: Optional[bool] = field(default=True, metadata={"help": "Whether to use warm initialization for the interaction module."})
    warm_init_value_im: Optional[float] = field(default=1e-4, metadata={"help": "Whether to use warm initialization for the value module."})
    debug_visual_probes: Optional[bool] = field(default=False, metadata={"help": "Whether to output visual probes for debugging."})
...
class LazySupervisedDataset(Dataset):
    ...
    def _convert_normal(self, data_dict):
        ...
        if 'image' in data_dict and data_dict['image'] is not None:
            ...
            modal = 'video'
            conversation[0]["value"] = conversation[0]["value"].replace('<image>', '<video>') # some data uses the image tag in conversations, but still passes the video key
...
def train(attn_implementation=None):
    ...
    # > ViSCoP specific config options
    config.include_visual_tokens = model_args.include_visual_tokens
    config.include_visual_probes = model_args.include_visual_probes
    config.num_visual_probes = model_args.num_visual_probes
    if model_args.interaction_module_layers == 'all': model_args.interaction_module_layers = None # if set to None, interaction modules will be placed after every layer. TODO: make this more intuitive
    config.interaction_module_layers = [int(x) for x in model_args.interaction_module_layers.split(',')] if model_args.interaction_module_layers is not None else None
    if model_args.interaction_module == 'cross_denoising_attention':
        config.interaction_module = 'cross_denoising_attention'
    elif model_args.interaction_module == 'cross_denoising_attention_split_heads':
        config.interaction_module = 'cross_denoising_attention_split_heads'
    elif model_args.interaction_module == 'cross_differential_attention':
        config.interaction_module = 'cross_differential_attention'
    else:
        config.interaction_module = 'cross_attention'

    config.debug_visual_probes =  model_args.debug_visual_probes if hasattr(model_args, 'debug_visual_probes') else False
    ...
    print('Loading model done in ' + str(time.time() - start) + 's')
    ...
    if training_args.lora_enable:
        ...
        target_lora_modules = find_all_linear_names(model) # ['down_proj', 'gate_proj', 'v_proj', 'up_proj', 'q_proj', 'k_proj', 'o_proj']
        lora_config = LoraConfig(
            r=training_args.lora_r,
            lora_alpha=training_args.lora_alpha,
            target_modules=f".*model\\.layers\\..*\\.({'|'.join(target_lora_modules)})$", # LoRA on LLM only
    ...
    if model_args.pretrained_lora_model is not None:
        rank0_print(f"Initializing LoRA adapters & other weights from {model_args.pretrained_lora_model}")
        if os.path.exists(os.path.join(model_args.pretrained_lora_model, 'non_lora_trainables.bin')):
            non_lora_trainables = torch.load(os.path.join(model_args.pretrained_lora_model, 'non_lora_trainables.bin'), map_location='cpu')

            _, unexpected = model.load_state_dict(non_lora_trainables, strict=False)
            assert len(unexpected) == 0, 'Not all non-LoRA weights were loaded from passed LoRA model!'

        if os.path.exists(os.path.join(model_args.pretrained_lora_model, 'adapter_model.bin')):
            adapter_model = torch.load(os.path.join(model_args.pretrained_lora_model, 'adapter_model.bin'), map_location='cpu')
            adapter_model = {k.replace('.weight', '.default.weight'): v for k, v in adapter_model.items()}

            _, unexpected = model.load_state_dict(adapter_model, strict=False)
            assert len(unexpected) == 0, 'Not all LoRA weights were loaded from passed LoRA model!'
    ...
    ###
    # Begin setting gradient status (enabled/disabled) of weights
    ###
    if model_args.vision_encoder is not None:
        ...
        if model.config.mm_projector_lr is None:
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = False
        else: # > disable gradients of projector based on which tokens will be passed
            if not model_args.include_visual_tokens:
                for p in model.get_model().mm_projector.parameters():
                    p.requires_grad = False

            if not model_args.include_visual_probes:
                for p in model.get_model().mm_projector_visual_probes.parameters():
                    p.requires_grad = False

        model.get_model().vision_encoder.vision_encoder.encoder.visual_probes.requires_grad = True # > ensure gradients on visual probes are enabled

            


        # > ensure interaction modules are trainable when VE is frozen
        for p in model.get_model().vision_encoder.vision_encoder.encoder.interaction_modules.parameters():
            p.requires_grad = True
        ###
        # End setting gradient status (enabled/disabled) of weights
        ###

        new_state_dict = {}
        if model_args.warm_init_interaction_module:
            warm_init = model_args.warm_init_value_im
            print("Scaling negative branchess with lower value:", warm_init)
        else:
            warm_init = 1.0
            print("No warm initialization for interaction modules.")

        for key, value in model.get_model().vision_encoder.vision_encoder.encoder.state_dict().items():
            if 'self_attn' in key:
                corresponding_key = key.replace('layers', 'interaction_modules').replace('self_attn.', '')

                if model_args.interaction_module == 'cross_denoising_attention' and ('q_proj' in key or 'v_proj' in key):
                    pos_key = corresponding_key.replace('q_proj', 'q_pos_proj').replace('v_proj', 'v_pos_proj')
                    neg_key = corresponding_key.replace('q_proj', 'q_neg_proj').replace('v_proj', 'v_neg_proj')
                    new_state_dict[pos_key] = value
                    new_state_dict[neg_key] = value * warm_init
                elif corresponding_key in model.get_model().vision_encoder.vision_encoder.encoder.state_dict():
                    new_state_dict[corresponding_key] = value

            elif 'interaction_modules' in key:
                if any(param in key for param in ['neg_scalers', 'lambda', 'subln', 'mix_logit']):
        ...
        _ = model.get_model().vision_encoder.vision_encoder.encoder.load_state_dict(new_state_dict)

        # > initialize visual probe projector with pretrained weights of visual projector
        _ = model.get_model().mm_projector_visual_probes.load_state_dict(model.get_model().mm_projector.state_dict())
        ###
        # End weight matching between SigLIP layers and cross-attention layers
        ###
        ...
        tokenizer.add_tokens([DEFAULT_IMAGE_TOKEN, STREAM_START_TOKEN, STREAM_END_TOKEN, DEFAULT_PROBE_TOKEN], special_tokens=True)
        model.config.image_token_index = tokenizer.convert_tokens_to_ids(DEFAULT_IMAGE_TOKEN)
        model.config.probe_token_index = tokenizer.convert_tokens_to_ids(DEFAULT_PROBE_TOKEN)

        vlprocessor = ViSCoP_Processor(
            vision_encoder.image_processor, 
            tokenizer, 
            include_visual_tokens=model_args.include_visual_tokens, 
            include_visual_probes=model_args.include_visual_probes, 
            num_visual_probes=model_args.num_visual_probes,
        )
    ...
    # select a Trainer
    trainer = ViSCoP_Trainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    print("==== Trainable Parameters ====")
    if local_rank == 0:
        for name, param in model.named_parameters():
            req_grad = param.requires_grad
            print(f"({req_grad}) {name}: {param.shape}, {param.numel()/1e6:.2f}M params")
