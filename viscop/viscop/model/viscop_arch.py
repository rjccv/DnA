...
from viscop.constants import IGNORE_INDEX, MODAL_INDEX_MAP, NUM_FRAMES
from viscop.model.encoder import build_vision_encoder
from viscop.model.projector import build_vision_projector, load_mm_projector
...
class ViSCoP_MetaModel:

    def __init__(self, config):
        super(ViSCoP_MetaModel, self).__init__(config)
        ...
        if hasattr(config, "vision_encoder") or hasattr(config, "mm_vision_encoder"):
            ...
            self.mm_projector = build_vision_projector(config, self.vision_encoder.hidden_size, standard_projector=True)
            self.mm_projector_visual_probes = build_vision_projector(config, self.vision_encoder.hidden_size)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if getattr(self, 'mm_projector', None) is None:
            self.mm_projector = build_vision_projector(self.config, standard_projector=True)
            self.mm_projector_visual_probes = build_vision_projector(self.config)
        else:
            # In case it is frozen by LoRA
            for p in self.mm_projector.parameters():
                p.requires_grad = True
            for p in self.mm_projector_visual_probes.parameters():
                p.requires_grad = True
...
class ViSCoP_MetaForCausalLM(ABC):
    ...
    def encode_images(
        ...
        mm_features, visual_probes = self.get_model().get_vision_encoder()(
            pixel_values=pixel_values,
            grid_sizes=grid_sizes,
            merge_sizes=merge_sizes,
        )
        mm_features = self.get_model().mm_projector(mm_features)
        visual_probes = self.get_model().mm_projector_visual_probes(visual_probes)
        return mm_features, visual_probes
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        debug = getattr(self.config, "debug_visual_probes", False)
        vision_encoder = self.get_vision_encoder()
        # NOTE: text-only situation
        if vision_encoder is None or pixel_values is None or input_ids.shape[1] == 1:
            return input_ids, None, attention_mask, position_ids, past_key_values, None, labels
        ...
        mm_features, visual_probes = self.encode_images(pixel_values, grid_sizes, merge_sizes)

        mm_features, visual_probes = mm_features.to(input_ids.device), visual_probes.to(input_ids.device)
        ...
        # 5. replace multimodal tokens with visual features & visual probes
        image_selected = (input_ids == self.config.image_token_index)
        if debug:
            print(
                "[debug_visual_probes] select images:",
                f"image_selected={int(image_selected.sum())},",
                f"expected={mm_features.shape[0]},",
                f"image_token_index={self.config.image_token_index},",
            )
        if image_selected.sum() == mm_features.shape[0]:
            inputs_embeds[image_selected] = inputs_embeds[image_selected] * 0.0 + mm_features

        probe_selected = (input_ids == self.config.probe_token_index)
        if debug:
            print(
                "[debug_visual_probes] select probes:",
                f"probe_selected={int(probe_selected.sum())},",
                f"expected={visual_probes.shape[0]},",
                f"probe_token_index={self.config.probe_token_index},",
                f"input_ids={tuple(input_ids.shape)}",
            )
        assert probe_selected.sum() == visual_probes.shape[0], f'Num probes {probe_selected.shape} selected:  {probe_selected.sum()} =! visual probes: {visual_probes.shape}'
        
        inputs_embeds[probe_selected] = inputs_embeds[probe_selected] * 0.0 + visual_probes
        ...
        return None, visual_probes, attention_mask, position_ids, past_key_values, inputs_embeds, labels
