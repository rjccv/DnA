...
def build_vision_projector_single(mm_hidden_size, hidden_size, projector_type):
...
def build_vision_projector(config, mm_hidden_size, delay_load=False, standard_projector=False, **kwargs):
    # videollama3 projector only support image-wise operation now, i.e., prohibit the temporal aggregation
    projector_type = getattr(config, 'mm_projector_type', 'linear')
    hidden_size = config.hidden_size

    return build_vision_projector_single(mm_hidden_size, hidden_size, projector_type)
