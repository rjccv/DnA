...
def INFERENCES(model_path):
    if model_path.endswith("videollama3_7b_local"):
    ...
    if 'viscop' in model_type.lower():
        assert hasattr(config, 'num_visual_probes'), "Model may not be a ViSCoP model. Config is missing num_visual_probes attribute."
        # NOTE: remote version
        ...
        # NOTE: local version
            ...
            return viscop.model_init, viscop.mm_infer
    elif model_type.lower() == 'videollama3_qwen2' and config.vision_encoder is not None:
