...
MODEL_NAMES={"work_dirs/egoexo/DnA_qwen2.5_7b_LLM_LoRA": "DnA"} #FIXME hardcoded paths.
...
def main():
    ...
    os.environ['GLOO_TIMEOUT_SECONDS'] = '7200'
    ...
    if args.benchmark == 'egoschema':
        model_name = MODEL_NAMES.get(args.model_path, "unknown_model")


    if global_rank == 0:
        results = sum(gathered_results, [])
        if args.benchmark == 'egoschema':
            metrics, infos = dataset.evaluate(results, model_name=model_name)
        else:
