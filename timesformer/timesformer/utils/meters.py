...
class TestMeter(object):
    ...
    def finalize_metrics(self, ks=(1, 5), eval_protocol=None):
        """
        ...
        if self.multi_label:
            ...
            if self.cfg.METRICS.ACCURACY == "mean_class":
                # Log and save mean-class diagnostics for datasets using mCA.
                conf_matrix, class_accuracies, mean_perclass_accuracy = (
                    metrics.mean_class_accuracies(
                        self.video_preds, self.video_labels, self.num_cls
                    )
                )
                mean_perclass_accuracy = float(
                    (mean_perclass_accuracy * 100.0).item()
                )
                valid_class_mask = ~torch.isnan(class_accuracies)
                num_valid_classes = int(valid_class_mask.sum().item())
                conf_matrix_np = conf_matrix.cpu().numpy()
                class_accuracies_np = class_accuracies.cpu().numpy()
                class_counts_np = conf_matrix.sum(dim=1).cpu().numpy()

                if eval_protocol is not None:
                    logger.info(
                        "Saving test set confusion matrix to: "
                        f"{self.cfg.OUTPUT_DIR}/testset_confusion_matrix_{eval_protocol}.npy"
                    )
                    np.save(
                        f"{self.cfg.OUTPUT_DIR}/testset_confusion_matrix_{eval_protocol}.npy",
                        conf_matrix_np,
                    ...
                    np.save(
                        f"{self.cfg.OUTPUT_DIR}/testset_class_accuracies_{eval_protocol}.npy",
                        class_accuracies_np,
                    )
                    np.save(
                        f"{self.cfg.OUTPUT_DIR}/testset_class_counts_{eval_protocol}.npy",
                        class_counts_np,
                    )
                else:
                    logger.info(
                        "Saving test set confusion matrix to: "
                        f"{self.cfg.OUTPUT_DIR}/testset_confusion_matrix.npy"
                    )
                    np.save(
                        f"{self.cfg.OUTPUT_DIR}/testset_confusion_matrix.npy",
                        conf_matrix_np,
                    ...
                    np.save(
                        f"{self.cfg.OUTPUT_DIR}/testset_class_accuracies.npy",
                        class_accuracies_np,
                    )
                    np.save(
                        f"{self.cfg.OUTPUT_DIR}/testset_class_counts.npy",
                        class_counts_np,
                    )

                self.stats["mCA"] = f"{mean_perclass_accuracy:.2f}"
                self.stats["mCA_num_classes"] = num_valid_classes
...
class ValMeter(object):
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self._cfg.DATA.MULTI_LABEL:
            ...
            if self._cfg.METRICS.ACCURACY == "mean_class":
                all_preds = torch.cat(self.all_preds, dim=0)
                all_labels = torch.cat(self.all_labels, dim=0)
                _conf_matrix, _class_accuracies, mean_perclass_accuracy = (
                    metrics.mean_class_accuracies(
                        all_preds,
                        all_labels,
                        self._cfg.MODEL.NUM_CLASSES,
                    )
                )
                top1_acc = float((mean_perclass_accuracy * 100.0).item())
                # mean_class_accuracies computes top-1 mCA only. Keep top-5 as sample accuracy.
                top5_err = self.num_top5_mis / self.num_samples
                top1_err = 100.0 - top1_acc
                stats["acc_mode"] = "mean_class"
                stats["top1_mc_acc"] = top1_acc
                stats["mCA"] = top1_acc
            else:
                top1_err = self.num_top1_mis / self.num_samples
                top5_err = self.num_top5_mis / self.num_samples
                stats["acc_mode"] = "sample"
