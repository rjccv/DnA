...
def mean_class_accuracies(preds, labels, num_classes):
    ...
    class_accuracies = torch.full(
        (num_classes,), float("nan"), device=preds.device
    )
    valid_mask = class_totals > 0
    class_accuracies[valid_mask] = (
        conf_matrix.diag()[valid_mask] / class_totals[valid_mask]
    )
    if valid_mask.any():
        mean_perclass_accuracy = class_accuracies[valid_mask].mean()
    else:
        mean_perclass_accuracy = torch.tensor(0.0, device=preds.device)
