...
class Smarthome(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        """
        Construct the Smarthome video loader with a split file.
        Supported formats:
        ```
        path_to_video_1 path_to_pose_1 label_1            # legacy txt
        path_to_video_1,path_to_pose_1,label_1            # csv
        path_to_video_1,label_1                           # csv/txt
        ...
        logger.info("Constructing {} {}...".format(dataset_name, mode))
        self._construct_loader()

    def _get_split_file(self):
        dataset_name = self.__class__.__name__
        split_root = (
            self.cfg.DATA.SMARTHOME_SPLIT_DIR
            if self.cfg.DATA.SMARTHOME_SPLIT_DIR
            else self.cfg.DATA.PATH_TO_DATA_DIR
        )
        candidates = [
            os.path.join(split_root, "{}.csv".format(self.mode)),
            os.path.join(split_root, "labels/{}_CS.txt".format(self.mode)),
        ]
        for path in candidates:
            if PathManager.exists(path):
                return path
        raise FileNotFoundError(
            "{} split file not found. Tried: {}".format(
                dataset_name,
                ", ".join(candidates)
            )
        )
    ...
    def _parse_split_row(self, row, line_no, path_to_file):
        ...
        if len(row) == 3:
            video_path, _, label = row
        elif len(row) == 2:
            video_path, label = row
        else:
            raise ValueError(
                "Invalid {} annotation at {}:{} -> {}".format(
                    self.__class__.__name__, path_to_file, line_no, row
                )
            )
        return video_path, int(label)
    ...
    def _construct_loader(self):
        ...
        with PathManager.open(path_to_file, "r") as f:
            if path_to_file.lower().endswith(".csv"):
                rows = csv.reader(f)
                parsed_rows = [self._parse_split_row(r, i + 1, path_to_file) for i, r in enumerate(rows)]
            else:
                parsed_rows = []
                ...
                for i, line in enumerate(f.read().splitlines()):
                    ...
                    parsed_rows.append(
                        self._parse_split_row(parts, i + 1, path_to_file)
                    )
